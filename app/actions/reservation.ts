...
export async function soumettreReservation(
  ...
  try {
    ...
    const lignes = cartItems.map((item) => {
      ...
      const prix_total = (item.type === 'equipement' ? prix_unitaire * nbJours : prix_unitaire) * item.quantite
    ...
    sendEmails({ clientData, startDate, endDate, nbJours, totalHT, besoinMontage, besoinLivraison, lignes: lignesEmail, reservationId })
...
async function sendEmails(data: {
  ...
  const { clientData, startDate, endDate, nbJours, totalHT, besoinMontage, besoinLivraison, lignes, reservationId } = data
  ...
  const gerantBody = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
      ...
      <tr>
        ...
        <td style="padding:6px 0;font-size:13px;color:#fff;">${startDate} → ${endDate} <span style="color:#888;">(${nbJours} jour${nbJours > 1 ? 's' : ''})</span></td>
      </tr>
      ${clientData.notes ? `<tr>
  ...
  const clientBody = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
      <tr>
        ...
        <td style="padding:5px 0;font-size:13px;color:#fff;">${startDate} → ${endDate} (${nbJours} jour${nbJours > 1 ? 's' : ''})</td>
      </tr>
      <tr>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:28px;background:#1a1718;padding:16px;border-left:3px solid #EC1C24;">
      <tr><td>
        ...
        <p style="margin:0;font-size:13px;color:#aaa;line-height:1.7;">
          ...
          • Pour toute question : <a href="tel:+41796512114" style="color:#EC1C24;">+41 79 651 21 14</a>
        </p>
      </td></tr>
    </table>
  `
  ...
  await Promise.all([
    sendEmail({
      to: 'info@vsonus.ch',
      subject: `🔔 Nouvelle demande de devis — ${clientData.nom}`,
      html: emailLayout(`Devis #${reservationId}`, gerantBody),
    }),
    sendEmail({
      to: clientData.email,
      subject: 'V-Sonus — Votre demande de devis a bien été reçue',
